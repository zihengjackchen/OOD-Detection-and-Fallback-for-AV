Camera images are collected at 4 Hz and placed in `sensor_data` folder in each scenario. Each `.pkl` file can be opened in python using the `pickle` package. It contains more than just the camera data, so the other sensor data can be ignored for now. Each `pkl` file stores the `input_data` variable. It can used as such:
```python
...
The demo images can be visualized as such:
```python
