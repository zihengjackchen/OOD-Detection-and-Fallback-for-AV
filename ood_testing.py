...
from Distribution import maha, autoencoder
import numpy as np
...
def demo(plot = False, pkl_dest = "/media/sheng/data4/projects/OOD/OOD-Detection/camera_input_source/SINGLE_AGENT_fi_lead_slowdown_00100/sensor_data/300.pkl"):
    with open("/media/sheng/data4/projects/OOD/OOD-Detection/camera_input_source/SINGLE_AGENT_fi_lead_slowdown_00100/sensor_data/300.pkl", "rb") as f:
...
def test_all_weather_on_single_frame(plot = False, pkl_dest = "/media/sheng/data4/projects/OOD/OOD-Detection/camera_input_source/SINGLE_AGENT_fi_lead_slowdown_00100/sensor_data/300.pkl"):
    ...
    filename = "/media/sheng/data4/projects/OOD/OOD-Detection/testing/single_frame_OOD_detection_autoencode.csv"
    ...
    for shade in range(1, 51):
        ...
        res = autoencoder.is_in_dist(shady_image)
        with open(filename, mode = 'a', newline='') as file:
            writer = csv.writer(file)
    ...
    for r in range(100, 2650, 50):
        ...
        res = autoencoder.is_in_dist(rainy_image)
        with open(filename, mode = 'a', newline='') as file:
            writer = csv.writer(file)
    ...
    for h in range(10, 265, 5):
        ...
        res = autoencoder.is_in_dist(hazy_image)
        with open(filename, mode = 'a', newline='') as file:
            writer = csv.writer(file)
...
def test_all_orig_frames_batch(base_dir):
    all_paths = []
    # Walk through the directory
    for root, dirs, files in os.walk(base_dir):
        # Check if current directory has the required subfolder structure
    ...
    for pkl_dest in all_paths:
        ...
        all_dicts.append(raw_rgb_dict)
        
    print(autoencoder.is_in_dist_batch(all_dicts))
